...
class Data:
    def __init__(
        self, sources, destinations, timestamps, edge_idxs, labels, deceiver_labels
    ):
        ...
        self.labels = labels
        self.deceiver_labels = deceiver_labels
...
def get_data(dataset_name, different_new_nodes_between_val_and_test=False):
    ...
    val_time, test_time = list(
        np.quantile(graph_df.ts, [0.70, 0.85])
    ...
    deceiver_labels = graph_df.deceiver_label.values
    timestamps = graph_df.ts.values

    full_data = Data(
        sources, destinations, timestamps, edge_idxs, labels, deceiver_labels
    )
    ...
    new_node_val_data = Data(
        ...
        deceiver_labels[new_node_val_mask],
    )
    ...
    new_node_test_data = Data(
        ...
        deceiver_labels[new_node_test_mask],
    )

    print(
        "The dataset has {} interactions, involving {} different unique nodes".format(
            full_data.n_interactions, full_data.n_unique_nodes
        )
    )
    print(
        "The training dataset has {} interactions, involving {} different unique nodes".format(
            train_data.n_interactions, train_data.n_unique_nodes
        )
    )
    print(
        "The validation dataset has {} interactions, involving {} different unique nodes".format(
            val_data.n_interactions, val_data.n_unique_nodes
        )
    )
    print(
        "The test dataset has {} interactions, involving {} different unique nodes".format(
            test_data.n_interactions, test_data.n_unique_nodes
        )
    )
    print(
        "The inductive validation dataset has {} interactions, involving {} different unique nodes".format(
            new_node_val_data.n_interactions, new_node_val_data.n_unique_nodes
        )
    )
    print(
        "The inductive test dataset has {} interactions, involving {} different unique nodes".format(
