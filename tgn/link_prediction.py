# This file is taken from TGN-Master with Avinash's original modifications
# It should be deprecated in the future
import math
import logging
import time
import sys
...
parser.add_argument(
    "--different_new_nodes",
    action="store_true",
    help="Whether val and test set use different unseen nodes (to test inductiveness)",
)
parser.add_argument(
...
parser.add_argument(
    "--n_runs", type=int, default=1, help="Number of runs for this script"
)
parser.add_argument(
...
parser.add_argument(
    "--memory_dim", type=int, default=172, help="Dimensions of the memory for each node"
)
parser.add_argument(
    "--neighbors", type=int, default=10, help="Number of neighbors to sample"
)
parser.add_argument(
    "--uniform",
    action="store_true",
...
(
    ...
    new_node_val_data,
    new_node_test_data,
) = get_data(DATA, different_new_nodes_between_val_and_test=args.different_new_nodes)
...
for i in range(args.n_runs):
    ...
    for epoch in range(NUM_EPOCH):
        ...
        for k in range(0, num_batch, args.backprop_every):
            ...
            for j in range(args.backprop_every):
                ...
                deceiver_prob = tgn.compute_deceiver_probability(
                    sources_batch,
                    destinations_batch,
                    timestamps_batch,
                    edge_idxs_batch,
                ...
                deceiver_label = torch.tensor(
                    deceiver_labels_batch, dtype=torch.float, device=device
                )

                loss += (
                    criterion(pos_prob.squeeze(), pos_label)
                    + criterion(neg_prob.squeeze(), neg_label)
                    + criterion(deceiver_prob.squeeze(), deceiver_label)
                )
        ...
        deceiver_val_ap, deceiver_val_auc = eval_deceiver_prediction(
            model=tgn,
            negative_edge_sampler=val_rand_sampler,
            data=val_data,
            n_nodes=n_nodes,
        ...
        logger.info(
            "transductive val auc: {}, inductive val auc: {}".format(
                val_auc, nn_val_auc
            )
        )
        logger.info(
        ...
        logger.info(
            "Deception val -- auc: {}, ap: {}".format(deceiver_val_auc, deceiver_val_ap)
        )
    ...
    n_nodes = len(set(train_data.sources))
    deceiver_test_ap, deceiver_test_auc = eval_deceiver_prediction(
        model=tgn,
        negative_edge_sampler=test_rand_sampler,
        data=test_data,
        n_nodes=n_nodes,
    ...
    logger.info(
        "Test statistics: Transductive -- auc: {}, ap: {}".format(test_auc, test_ap)
    )
    logger.info(
        "Test statistics: Inductive -- auc: {}, ap: {}".format(nn_test_auc, nn_test_ap)
    )
    logger.info(
        "Deception Test statistics: -- auc: {}, ap: {}".format(
            deceiver_test_auc, deceiver_test_ap
        )
    )
